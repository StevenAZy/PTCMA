...
class ACMIL_MHA(nn.Module):
    def __init__(self, n_classes, n_token=5, n_masked_patch=10, mask_drop=0.6):
        super(ACMIL_MHA, self).__init__()
        self.dimreduction = DimReduction(1024, 512)
        self.sub_attention = nn.ModuleList()
        for i in range(n_token):
            self.sub_attention.append(MutiHeadAttention(512, 8, n_masked_patch=n_masked_patch, mask_drop=mask_drop))
        self.bag_attention = MutiHeadAttention_modify(512, 8)
        self.q = nn.Parameter(torch.zeros((1, n_token, 512)))
        nn.init.normal_(self.q, std=1e-6)
        self.n_class = n_classes

        self.classifier = nn.ModuleList()
        for i in range(n_token):
            self.classifier.append(Classifier_1fc(512, n_classes, 0.0))
        self.n_token = n_token
        self.Slide_classifier = Classifier_1fc(512, n_classes, 0.0)
    ...
    def forward(self, input):
        ...
        logits = torch.mean(output_stack, dim=0)
...
class ABMIL(nn.Module):
    def __init__(self, n_classes, D=128, droprate=0):
        super(ABMIL, self).__init__()
        self.dimreduction = DimReduction(1024, 512)
        self.attention = Attention_Gated(512, D, 1)
        self.classifier = Classifier_1fc(512, n_classes, droprate)
...
class ACMIL_GA(nn.Module):
    def __init__(self, n_classes, D=128, droprate=0, n_token=5, n_masked_patch=10, mask_drop=0):
        super(ACMIL_GA, self).__init__()
        self.dimreduction = DimReduction(1024, 512)
        self.attention = Attention_Gated(512, D, n_token)
        self.classifier = nn.ModuleList()
        for i in range(n_token):
            self.classifier.append(Classifier_1fc(512, n_classes, droprate))
        self.n_masked_patch = n_masked_patch
        self.n_token = n_token
        self.Slide_classifier = Classifier_1fc(512, n_classes, droprate)
        self.mask_drop = mask_drop
    ...
    def forward(self, x): ## x: N x L
        ...
        output = self.Slide_classifier(bag_feat)
